...
function getCapabilityNotes(options = {}) {
  ...
  if (available.some(t => t.name === 'image_generate')) {
    const chatImageLimit = Number(process.env.CHAT_MAX_IMAGE_GENERATIONS || 4);
    capabilities.push(`Image generation is AVAILABLE, but it is hard-limited to ${chatImageLimit} image(s) per chat turn to prevent runaway credit spend. Use image_generate to create visuals for websites, social posts, flyers, etc. For batch image requests, never request more than ${chatImageLimit} images in one turn. Prompts are auto-enhanced for quality, but YOU should still write detailed prompts: describe subject, lighting, camera/lens, composition, colors, mood. If the user asks for an image of you, the current Bloomie, or this employee, generate it directly and describe your own employee identity/visual style; the platform will attach your saved agent avatar_url as the reference image when available. Do not invent a different person for self-images. Set engine=openrouter when OpenRouter is the preferred provider, engine=gpt for social/flyers/thumbnails when OpenAI images are configured, engine=gemini for website heroes/blog images or reference-image consistency. Default style is PHOTOREALISTIC — never produce cartoon or illustrated unless the user asks.`);
  }
