...
import type { SearchFacebookGroupsInput } from "../schemas/inputs.js";
import type { GroupMember, ScrapeResponse } from "../types.js";

export async function searchFacebookGroups(
  params: SearchFacebookGroupsInput
): Promise<ScrapeResponse<GroupMember>> {
  const errors: string[] = [];
  ...
  if (params.group_url) {
    ...
    try {
      ...
      errors.push(
        `Could not access the Facebook group. ${friendlyError(err)}. ` +
    ...
    warnings.push(
      `To find Facebook groups for "${params.query}", I need browser access while you're logged into Facebook.\n\n` +
        `Here's what I'll do:\n` +
