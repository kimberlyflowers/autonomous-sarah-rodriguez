...
export async function scrapeUrl(params) {
    ...
    try {
                ...
                if (elements.length >= 3) {
                    ...
                    elements.each((_i, el) => {
                        ...
                        for (const col of params.columns) {
                            ...
                            const labelRegex = new RegExp(`${colLower}[:\\s]*([^\\n,|]+)`, "i");
                            ...
                            if (colLower.includes("url") ||
                                ...
                                const mailto = $(el)
                                    ...
                                    .attr("href");
                                if (mailto)
                                    rowData[col] = mailto.replace("mailto:", "");
                            }
                                ...
                                const tel = $(el)
                                    ...
                                    .attr("href");
                                if (tel)
                                    rowData[col] = tel.replace("tel:", "");
                                else {
                                    // Regex for phone numbers
                                    const phoneMatch = text.match(/\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}/);
                                    if (phoneMatch)
                                        rowData[col] = phoneMatch[0];
                                }
                            }
        ...
        if (results.length === 0) {
            warnings.push("Could not detect a structured layout. Extracted raw text blocks instead. " +
        ...
        if (results.length === 0) {
            errors.push("No extractable data found on this page. The page might be JavaScript-rendered " +
