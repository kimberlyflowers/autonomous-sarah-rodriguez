...
export async function searchLinkedIn(params) {
    ...
    if (!apiKey) {
        errors.push("PhantomBuster API key not configured. " +
    ...
    if (!agentId) {
        errors.push("PhantomBuster LinkedIn agent not configured. " +
    ...
    try {
        ...
        if (params.location)
            searchParts.push(params.location);
        if (params.job_title)
            searchParts.push(`title:${params.job_title}`);
        if (params.company)
            searchParts.push(`company:${params.company}`);
        const linkedinSearchUrl = `https://www.linkedin.com/search/results/people/?keywords=${encodeURIComponent(searchParts.join(" "))}`;
        // Launch the PhantomBuster agent with the search URL
        const launchResponse = await fetchJson(`${PHANTOM_API}/agents/launch`, {
            method: "POST",
            headers: {
        ...
        if (!launchResponse.containerId) {
            errors.push("PhantomBuster agent launch failed. " +
                "The LinkedIn agent may be busy or misconfigured.");
            return makeResponse(params, results, errors, warnings);
        }
        // Poll for results (PhantomBuster runs async)
        warnings.push("LinkedIn search launched via PhantomBuster. " +
        ...
        try {
            const outputResponse = await fetchJson(`${PHANTOM_API}/agents/fetch-output`, {
                method: "POST",
                headers: {
                    "X-Phantombuster-Key": apiKey,
                },
            ...
            if (outputResponse?.output) {
                ...
                const lines = typeof outputResponse.output === "string"
                    ? JSON.parse(outputResponse.output)
                    : outputResponse.output;
                if (Array.isArray(lines)) {
                    for (const line of lines.slice(0, params.limit)) {
                        results.push({
                            name: [line.firstName, line.lastName].filter(Boolean).join(" ") ||
                                line.name ||
                                "Unknown",
            ...
            warnings.push("No cached results available yet. The search is running. " +
...
function makeResponse(params, results, errors, warnings) {
    return {
        query: params.query,
...
//# sourceMappingURL=search-linkedin.js.map