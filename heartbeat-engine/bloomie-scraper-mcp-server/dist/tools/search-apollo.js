...
export async function searchApollo(params) {
    ...
    if (!apiKey) {
        errors.push("Apollo.io API key not configured. " +
    ...
    try {
        ...
        const data = await fetchJson(`${APOLLO_API}/mixed_people/search`, {
            method: "POST",
            headers: {
                "X-Api-Key": apiKey,
            },
        ...
        if (results.length === 0) {
            errors.push(`Apollo returned no results for "${params.query}"` +
        ...
        if (data?.pagination?.total_entries) {
            warnings.push(`Apollo found ${data.pagination.total_entries} total matches. ` +
...
//# sourceMappingURL=search-apollo.js.map