...
async function scrapeYellowpages(query, location, limit) {
    const errors = [];
    const results = [];
    try {
        ...
        $(".result").each((_i, el) => {
            ...
            results.push({
                ...
                state: null,
                category: $(el).find(".categories a").first().text().trim() || null,
        ...
        if (results.length === 0) {
            errors.push(`Yellowpages returned no results for "${query}" in "${location}". ` +
...
async function scrapeYelp(query, location, limit) {
    const errors = [];
    const results = [];
    try {
        ...
        $('[data-testid="serp-ia-card"]').each((_i, el) => {
            if (results.length >= limit)
                return false;
            const name = $(el).find("a[class*='css'] span").first().text().trim() ||
        ...
        if (results.length === 0) {
            errors.push(`Yelp returned no results for "${query}" in "${location}". ` +
...
export async function searchBusinesses(params) {
    ...
    if (params.source === "yellowpages" || params.source === "both") {
        const yp = await scrapeYellowpages(params.query, params.location, params.source === "both" ? halfLimit : params.limit);
        allResults.push(...yp.results);
        allErrors.push(...yp.errors);
    }
    if (params.source === "yelp" || params.source === "both") {
        const yelp = await scrapeYelp(params.query, params.location, params.source === "both" ? halfLimit : params.limit);
    ...
    if (deduped.length < allResults.length) {
        warnings.push(`Removed ${allResults.length - deduped.length} duplicate listings.`);
    }
    // Add upsell hint if results are limited
    if (deduped.length > 0) {
        warnings.push("💡 Want verified emails and phone numbers for these businesses? " +
...
//# sourceMappingURL=search-businesses.js.map